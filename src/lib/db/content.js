...
export async function saveContent({
  ...
  try {
    const rows = await sql`
      INSERT INTO generated_content (user_id, profile_id, kind, topic, platforms, title, markdown, data)
      VALUES (
        ${userId},
        ${profileId},
        ${kind},
      ...
      RETURNING id, profile_id, kind, topic, platforms, title, markdown, data, created_at`;
    return rows[0] || null;
  } catch {
    return null;
  }
...
export async function listContent(userId = null, { kind = null, limit = 30 } = {}) {
  ...
  try {
    if (userId) {
      return kind
        ? await sql`
            SELECT id, profile_id, kind, topic, platforms, title, markdown, data, created_at

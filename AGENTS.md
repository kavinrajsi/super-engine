...
**Site health snapshot — backlinks + live SERP** (`src/lib/seo/backlinks.js` + `serp.js` + `keywords.js` + `src/app/api/seo/{backlinks,serp}/*` + `src/app/scan/snapshot-cards.js`): the scan **Overview panel** leads with a consolidated snapshot — SEO score + Lighthouse performance (live PageSpeed) + **backlinks** + **live SERP rank**. Backlinks/SERP come from a **pluggable external provider** and **no-op to `{configured:false}`** (UI shows a "connect a provider" card) until one is wired — set `BACKLINKS_PROVIDER`/`SERP_PROVIDER` or `DATAFORSEO_LOGIN`/`DATAFORSEO_PASSWORD` (a DataForSEO reference impl is scaffolded but inactive without creds; flesh out the POST body to go live). On-demand routes mirror `/api/pagespeed`, are SSRF-guarded, and **reuse a ≤24h snapshot** (`backlinks_snapshots`/`serp_snapshots`, `latest*Snapshot`) to avoid repeat **paid** calls — gate live refresh behind a button, never auto-fire paid fetches. SERP keywords are derived on-page via `candidateKeywords` (no GSC needed). Note: GSC's `position` is *self-reported* avg rank; this SERP feature is the *live* Google rank.
...
Durable background jobs for large sites, and **GBP Monitor** (Local SEO via Google Business Profile API — needs a Google OAuth client + the user's owned business + Google's gated Business Profile API approval). The backlinks + live-SERP **provider** is scaffolded but not wired to a real provider yet (set `DATAFORSEO_*` and complete the POST impl). See `~/.claude/plans/` and project memory for the current plan.

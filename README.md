...
Next.js 16 (App Router, React 19, React Compiler, Turbopack) · JavaScript · Tailwind CSS v4 · shadcn/ui (Base UI) · cheerio · puppeteer-core · Vercel AI SDK (v6) + zod · recharts · next-themes · deployed on **Vercel**.
...
| `BROWSER_WS_ENDPOINT` | Headless rendering (prod) | A remote browser CDP endpoint (Browserless / Browserbase / Cloudflare). |
...
src/
  app/
    page.js              Home (URL input + link to /tools)
    layout.js            Root layout (next-themes)
...
The scan pipeline lives in `src/lib/seo/analyze.js` (`runScan`), which produces a single `result` object consumed by every dashboard panel and export.
